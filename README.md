...
The agent regularly runs one of the programs (a program can be run if all the reactant conditions are met and none of the inhibitor conditions are met).

The application can be compiled using a compiler that supports C++ 17.
E.g. when using the g++ compiler, add the parameter -std=c++17

The program code was designed to be cross-platform wherever possible and can be compiled for various operating systems. If you don't currently have a compiler installed, you can also use an online tool that allows you to specify the C++ standard version, such as OnlineGDB.

After launching the simulator, a menu appears where you can select the desired action: display information about agents, start/stop a simulation, display the device status, manually increase or decrease the temperature on the thermometer, turn verbose mode on or off, or exit the application.
